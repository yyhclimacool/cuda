 ...
 *
 * `nvtx3::unique_range` is similar to `nvtx3::scoped_range`, with a few key
 * differences:
 * - `unique_range` objects can be destroyed in any order whereas `scoped_range`
 * objects must be destroyed in exact reverse creation order
 ...
 *
 * There is extra overhead associated with `unique_range` constructs and
 * therefore use of `nvtx3::scoped_range_in` should be preferred.
 *
 * \section MARKS Marks
 *
 ...
 * static nvtx3::named_category_in<my_domain> static_category{42, "my
 * category"};
 *
 * // Or use construct on first use:
...
NVTX3_INLINE_IF_REQUESTED namespace NVTX3_VERSION_NAMESPACE {
  ...
  template <typename Unused> struct always_false : std::false_type {};
  ...
  struct is_c_string<T,
                     typename std::enable_if<
                         ...
                         std::is_convertible<T, wchar_t const *>::value>::type>
      : std::true_type {};
  ...
  public:
    ...
    template <
        typename D = global,
        typename std::enable_if<!detail::is_c_string<decltype(D::name)>::value,
                                int>::type = 0>
    static domain const &get() noexcept {
      NVTX3_STATIC_ASSERT(detail::always_false<D>::value,
                          "Type used to identify an NVTX domain must contain a "
                          "static constexpr member "
                          "called 'name' of type const char* or const wchar_t* "
                          "-- 'name' member is not "
    ...
    template <
        typename D = global,
        typename std::enable_if<!detail::has_name<D>::value, int>::type = 0>
    static domain const &get() noexcept {
      NVTX3_STATIC_ASSERT(detail::always_false<D>::value,
                          "Type used to identify an NVTX domain must contain a "
                          "static constexpr member "
                          "called 'name' of type const char* or const wchar_t* "
                          "-- 'name' member is missing");
      static domain const unused;
      return unused; // Function must compile for static_assert to be triggered
    }
#else
    template <typename D = global> static domain const &get() noexcept {
      static domain const d(D::name);
      return d;
    }
#endif
  ...
  private:
    ...
    /**
     * @brief Intentionally avoid calling nvtxDomainDestroy on the `domain`
     * object.
     *
   ...
   *
   */
  template <> inline domain const &domain::get<domain::global>() noexcept {
  ...
  public:
    ...
    constexpr color(argb argb_) noexcept
        : color{from_bytes_msb_to_lsb(argb_.alpha, argb_.red, argb_.green,
                                      argb_.blue)} {}
  ...
  public:
     ...
     *
     * Creates a `named_category_in<D>` with name and id specified by the
     * contents of a type `C`. `C::name` determines the name and `C::id`
    ...
    /**
     * @brief Overload of `named_category_in::get` to provide a clear compile
     * error when `C` has the required `name` and `id` members, but they are not
     * the required types.  `name` must be directly convertible to `char const*`
     * or `wchar_t const*`, and `id` must be `uint32_t`.
     */
    template <typename C, typename std::enable_if<
                              !detail::is_c_string<decltype(C::name)>::value ||
                                  !detail::is_uint32<decltype(C::id)>::value,
    ...
    static named_category_in const &get() noexcept {
      NVTX3_STATIC_ASSERT(detail::is_c_string<decltype(C::name)>::value,
                          "Type used to name an NVTX category must contain a "
                          "static constexpr member "
                          "called 'name' of type const char* or const wchar_t* "
                          "-- 'name' member is not "
                          "convertible to either of those types");
      NVTX3_STATIC_ASSERT(
          detail::is_uint32<decltype(C::id)>::value,
          "Type used to name an NVTX category must contain a static constexpr "
          "member "
    ...
    /**
     * @brief Overload of `named_category_in::get` to provide a clear compile
     * error when `C` does not have the required `name` and `id` members.
     */
    template <typename C, typename std::enable_if<!detail::has_name<C>::value ||
...
#else
    template <typename C> static named_category_in const &get() noexcept {
      static named_category_in const cat(C::id, C::name);
      return cat;
    }
#endif
  ...
  private:
    // Default constructor is only used internally for static_assert(false)
    // cases.
    named_category_in() noexcept : category{0} {}

  public:
    /**
     * @brief Construct a `named_category_in` with the specified `id` and
     * `name`.
     *
   ...
   *
   * @tparam D Type containing `name` member used to identify the `domain` to
   * which the `registered_string_in` belongs. Else, `domain::global` to
   * indicate that the global NVTX domain should be used.
   */
  template <typename D = domain::global> class registered_string_in {
  public:
#if NVTX3_USE_CHECKED_OVERLOADS_FOR_GET
    /**
     * @brief Returns a global instance of a `registered_string_in` as a
     * function local static.
     *
     ...
     * @return Reference to a `registered_string_in` associated with the type
     * `M`.
     */
    ...
    /**
     * @brief Overload of `registered_string_in::get` to provide a clear compile
     * error when `M` has a `message` member that is not directly convertible to
     * either `char const*` or `wchar_t const*`.
     */
    template <typename M, typename std::enable_if<
                              !detail::is_c_string<decltype(M::message)>::value,
                              int>::type = 0>
    static registered_string_in const &get() noexcept {
      NVTX3_STATIC_ASSERT(detail::always_false<M>::value,
                          "Type used to register an NVTX string must contain a "
                          "static constexpr member "
                          "called 'message' of type const char* or const "
                          "wchar_t* -- 'message' member is "
    ...
    static registered_string_in const &get() noexcept {
      NVTX3_STATIC_ASSERT(detail::always_false<M>::value,
                          ...
                          "static constexpr member "
                          "called 'message' of type const char* or const "
                          "wchar_t* -- 'message' member "
                          "is missing");
      static registered_string_in const unused;
      return unused; // Function must compile for static_assert to be triggered
    }
#else
    template <typename M> static registered_string_in const &get() noexcept {
      static registered_string_in const regstr(M::message);
      return regstr;
    }
#endif
    ...
    /**
     * @brief Constructs a `registered_string_in` from the specified `msg`
     * string.
     *
  ...
  public:
     ...
     */
    constexpr message(nvtxMessageType_t const &type,
  ...
  public:
     ...
     */
    constexpr payload(nvtxPayloadType_t const &type,
   ...
   *
   * // Using the nvtx3 namespace in a local scope makes the syntax more
   * succinct: using namespace nvtx3; scoped_range r{payload{42}, category{1},
   * "message"};
   * \endcode
   *
   */
  ...
  public:
    ...
    NVTX3_CONSTEXPR_IF_CPP14 explicit event_attributes(
   ...
   * \endcode
   */
  template <class D = domain::global> class scoped_range_in {
  public:
    /**
     ...
     * \endcode
     *
     * @param[in] args Arguments to used to construct an `event_attributes`
     ...
     *
     * `scoped_range_in` must follow RAII semantics to guarantee proper push/pop
     * semantics.
     *
     */
  ...
  /// @cond internal
  template <typename D = domain::global> class optional_scoped_range_in {
  public:
    optional_scoped_range_in() = default;

    void begin(event_attributes const &attr) noexcept {
#ifndef NVTX_DISABLE
      // This class is not meant to be part of the public NVTX C++ API and
      // should only be used in the `NVTX3_FUNC_RANGE_IF` and
      // `NVTX3_FUNC_RANGE_IF_IN` macros. However, to prevent developers from
      // misusing this class, make sure to not start multiple ranges.
      if (initialized) {
        return;
      }
    ...
    operator=(optional_scoped_range_in const &) = delete;
   ...
   */
  inline constexpr bool operator==(range_handle lhs,
                                   range_handle rhs) noexcept {
   ...
   *
   * `nvtx3::start_range(...)` is equivalent to `nvtx3::start_range_in<>(...)`
   ...
   * preferred unless one is unable to tie the range to the lifetime of an
   * object.
   *
   * Example:
   * \code{.cpp}
   ...
   * \code{cpp}
   * // Manually begin a range
   * nvtx3::range_handle h = nvtx3::start_range_in<D>("msg",
   * nvtx3::rgb{127,255,0});
   * ...
   ...
   * to which the range belongs. Else, `domain::global` to indicate that the
   * global NVTX domain should be used.
   * @param args[in] Variadic parameter pack of the arguments for an
   * `event_attributes`.
   ...
   * must be the same that was specified on the associated `start_range_in`
   * call.
   */
  template <typename D = domain::global>
   ...
   * - `unique_range` objects can be destroyed in an order whereas
   * `scoped_range` objects must be destroyed in exact reverse creation order
   * - `unique_range` can start and end on different threads
   * - `unique_range` is moveable
   * - `unique_range` objects can be constructed as heap objects
   *
   ...
   * indicate that the global NVTX domain should be used.
   */
  template <typename D = domain::global> class unique_range_in {
  public:
    /**
     * @brief Construct a new unique_range_in object with the specified event
     * attributes
     *
     * Example:
     * \code{cpp}
     * nvtx3::event_attributes attr{"msg", nvtx3::rgb{127,255,0}};
     * nvtx3::unique_range_in<my_domain> range{attr}; // Creates a range with
     * message contents
     *                                            // "msg" and green color
     * \endcode
     *
    ...
    /// Copy assignment is not allowed to prevent multiple objects from owning
    /// the same range handle
  ...
  private:
    struct end_range_handle {
      using pointer =
          range_handle; /// Override the pointer type of the unique_ptr
   ...
   *
   * Unlike a "range" which has a beginning and an end, a marker is a single
   * event in an application, such as detecting a problem:
   *
   * \code{.cpp}
   * bool success = do_operation(...);
   * if (!success) {
   ...
   * \endcode
   *
   * Note that nvtx3::mark_in<D> is a function, not a class like
   * scoped_range_in<D>.
   *
   * @tparam D Type containing `name` member used to identify the `domain`
   * to which the `unique_range_in` belongs. Else, `domain::global` to
   ...
   *
   */
  template <typename... Args> inline void mark(Args const &...args) noexcept {
#ifndef NVTX_DISABLE
...
#define NVTX3_V1_FUNC_RANGE_IN(D)                                              \
  static ::nvtx3::v1::registered_string_in<D> const nvtx3_func_name__{         \
      __func__};                                                               \
  static ::nvtx3::v1::event_attributes const nvtx3_func_attr__{                \
      nvtx3_func_name__};                                                      \
 ...
 * \endcode
 */
#define NVTX3_V1_FUNC_RANGE()                                                  \
  NVTX3_V1_FUNC_RANGE_IN(::nvtx3::v1::domain::global)
 ...
 * generated.
 */
#define NVTX3_V1_FUNC_RANGE_IF(C)                                              \
  NVTX3_V1_FUNC_RANGE_IF_IN(::nvtx3::v1::domain::global, C)
...
/* Add functionality for new minor versions here, by copying the above section
 * enclosed in #ifndef NVTX3_CPP_DEFINITIONS_Vx_y, and incrementing the minor
 * version.  This code is an example of how additions for version 1.2 would
 * look, indented for clarity.  Note that the versioned symbols and macros are
 * always provided, and the unversioned symbols are only provided if
 * NVTX3_INLINE_THIS_VERSION was defined at the top of this header.
 *
 * \code{.cpp}
 ...
 *
 *     // If inlining, make aliases for the macros with the version number
 * omitted #if defined(NVTX3_INLINE_THIS_VERSION) #define NVTX3_NEW_MACRO_A
 * NVTX3_V1_NEW_MACRO_A #define NVTX3_NEW_MACRO_B NVTX3_V1_NEW_MACRO_B #endif
 * #endif // NVTX3_CPP_DEFINITIONS_V1_2
 * \endcode
 */

/* Undefine all temporarily-defined unversioned macros, which would conflict
 * with subsequent includes of different versions of this header. */
